 ...
 * ../../../agents-everywhere-starter-kit/apps/web/src/lib/transcription-session.ts
 * (MIT, same workspace). The only change is the relative import below gaining
 * a `.ts` extension, per this repo's house rule. Everything else — including
 * this comment block — is the original author's.
 *
 ...
 *
 * One session handles one speaker. T5 opens one of these per LiveKit
 * participant track (see ../participant-pipeline.ts) — no stereo merge here,
 * unlike the starter kit's tab-plus-mic capture, because LiveKit already
 * hands each participant its own track.
 */
import { TRANSCRIBE_MODEL } from "./realtime-config.ts";
...
export type TranscriptionSessionOptions = {
  /** Ephemeral client secret from the token-server — never the API key. */
  secret: string;
